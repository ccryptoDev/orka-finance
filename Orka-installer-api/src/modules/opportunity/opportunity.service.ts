...
export class OpportunityService {
  ...
  public  async getLoanDetails(id:string) {
    try {
      ...
      const contractReviewComments = await entityManager.query(
        `
          select
            comments,
            "updatedAt"
          from
            tblsalescontractreview
          WHERE
            loanid = $1 AND status = 'needadditionalinfo'
        `,
        [id]
      );
      const milestoneReviewComments = await entityManager.query(
        `
          select
            comments,
            milestone,
      ...
      const selectedProduct = await entityManager.query(
        `
          select
            t2."productId",
            t2.name,
            ...
            t2."mpfCheckWItcPrepay", 
            t2."mpfCheckWOPrepay",
            lm."financingApproved"  
          from
            tblloan tln
          join
            tblproduct t2 on CAST(tln."productId"  as int) = t2."productId"
          join
            tbllendinglimit lm on lm."loanId" = tln.id AND lm."productId" = tln."productId"
          where
            tln.id = $1
        `,
        [id]
